...
class MainClass:
	...
	def rotateHumanRep(self,humanRep,j):
		...
		tmp1 = np.concatenate([tmp, tmp[0:1, :]], axis=0)
	...
	def run(self):
		actions = 4
		comRep = np.load('CascadeCodeComputer.npy')
		humRep=np.load('CascadeCodeHuman.npy')
		size = comRep.shape[0]

		validation_data_comp = np.load('CascadeCodeComputerTest.npy')
		validation_data_hum = np.load('CascadeCodeHumanTest.npy')
		...
		for i in range(comRep.shape[2]):
			for j in range(4):
				...
				if((trainingIteration+1) % self.saveRate == 0):
					filenameNetworkParameters = 'Network11x11.h5'
