...
class MainClass:
	...
	def __init__(self):
		self.graphix = False
		self.networkName = 'Network11x11.h5'
		self.run()
	...
	def run(self):
		Generator = Generate()
		importNetwork = tensorflow.keras.models.load_model(self.networkName)
		...
		for prob in testProbs:
			...
			humRep=np.load('CascadeCodeHumanTest.npy')
			comRep=np.load('CascadeCodeComputerTest.npy')
